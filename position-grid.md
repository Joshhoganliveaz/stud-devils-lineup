# Stud Devils Position Grid

## Game Format
- 6 innings, coach pitch, 13 players
- 10 fielders per inning (6 infield + 4 outfield), 3 bench
- 8 players play 5 innings (bench once), 5 players play 4 innings (bench twice)

## Rules Applied
- P + 1B combined: max 2 innings per player (forfeiture penalty)
- Max 2 consecutive innings in infield OR outfield (bench resets count)
- No consecutive bench innings
- Jacob never plays 1B
- Julian plays 2B when in the infield

## Pitching Rotation
- Inn 1: Diego
- Inn 2: Diego
- Inn 3: Cruz
- Inn 4: Cruz
- Inn 5: Donovan
- Inn 6: Donovan

## Catcher Rotation
- Inn 1: Michael
- Inn 2: Theo
- Inn 3: Bruce
- Inn 4: Remy
- Inn 5: Theo
- Inn 6: Michael

## 1B Rotation
- Inn 1: Kai
- Inn 2: Kai
- Inn 3: Maddox
- Inn 4: Maddox
- Inn 5: Emmett
- Inn 6: Emmett

## Bench Schedule
- Inn 1: Donovan, Jacob, Remy
- Inn 2: Maddox, Michael, Bruce
- Inn 3: Theo, Emmett, Julian
- Inn 4: Kai, Juli, Michael
- Inn 5: Cruz, Diego, Remy
- Inn 6: Theo, Juli, Bruce

## Full Position Grid

| Player  | Inn 1 | Inn 2 | Inn 3 | Inn 4 | Inn 5 | Inn 6 |
|---------|-------|-------|-------|-------|-------|-------|
| Donovan | Bench | SS    | SS    | LF    | P     | P     |
| Cruz    | SS    | LC    | P     | P     | Bench | LC    |
| Maddox  | LF    | Bench | 1B    | 1B    | LF    | 3B    |
| Theo    | RF    | C     | Bench | RF    | C     | Bench |
| Kai     | 1B    | 1B    | LF    | Bench | SS    | 2B    |
| Diego   | P     | P     | RC    | SS    | Bench | SS    |
| Jacob   | Bench | RF    | RF    | 3B    | 3B    | RC    |
| Emmett  | 3B    | 3B    | Bench | RC    | 1B    | 1B    |
| Juli    | LC    | LF    | 2B    | Bench | RF    | Bench |
| Remy    | Bench | RC    | LC    | C     | Bench | LF    |
| Julian  | 2B    | 2B    | Bench | LC    | 2B    | RF    |
| Michael | C     | Bench | 3B    | Bench | LC    | C     |
| Bruce   | RC    | Bench | C     | 2B    | RC    | Bench |

## P+1B Accounting
- Diego: P(1) + P(2) = 2
- Cruz: P(3) + P(4) = 2
- Donovan: P(5) + P(6) = 2
- Kai: 1B(1) + 1B(2) = 2
- Maddox: 1B(3) + 1B(4) = 2
- Emmett: 1B(5) + 1B(6) = 2
- All others: 0

## Bench-Once (Play 5)
Cruz, Maddox, Kai, Donovan, Diego, Emmett, Jacob, Julian

## Bench-Twice (Play 4)
Theo, Juli, Remy, Michael, Bruce
